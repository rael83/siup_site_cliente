...
if (!class_exists('Enquete_model'))
{
	class Enquete_model extends Meumodelo
	{
		...
		public function Filtro($semOrder = false)
		{
			...
			if(!empty($datainicio))
			{
				$filtro .= " AND datainicio BETWEEN '{$datainicioinicio}' AND '{$datainiciofim}' ";
			}
			$datafiminicio = Componente::GetFiltro("datafiminicio");
			$datafimfim = Componente::GetFiltro("datafimfim");
			if(!empty($datafim))
			{
				$filtro .= " AND datafim BETWEEN '{$datafiminicio}' AND '{$datafimfim}'";
			}
			...
			{
				$filtro .= " AND cadastradoem BETWEEN '{$cadastradoeminicio}' AND '{$cadastradoemfim}'";
			}
			if(!empty($filtro))
			{
