...
final class WP_Route{
    ...
    public static function get($route, $callable){
    	$r = self::instance();
    	$r->addRoute('GET', $route, $callable, array(
		    'code'     => 200,
	    ));
    }
    ...
    private function hook(){
    	if(!$this->hooked){
			add_filter('init', array('WP_Route', 'onInit'), -1000, 0);
			$this->hooked = TRUE;
		}
    }
    ...
    public function requestURI(){
    	if(Componente::is_localhost())
	    {
		   $aux = $_SERVER['REQUEST_URI'];
		    $aux = str_replace("clubinhodafesta.com.br","", $aux);
		    return ltrim($aux, '/');
	    }
	    else
    ...
    public function handle(){
    	...
    	foreach($routes as $key => $route){
    		// First, filter routes that do not have equal tokenized length
    		if(count($this->tokenize($route->route)) !== count($tokenizedRequestURI)){
    			unset($routes[$key]);
    			continue;
    		}
    		$len = strripos($route->route,'/');
    		if($len === false)
		        $rota = substr($route->route, 0);
    		else
			    $rota = substr($route->route, 0, $len);
		    $len = strripos($route->route,'/');
		    if($len === false)
		        $uri = substr($requestURI, 0);
		    else
			    $uri = substr($requestURI, 0, $len);
		    if(strcmp($rota, $uri) != 0)
		    {
			    unset($routes[$key]);
			    continue;
		    }
