...
if (!class_exists('Documento_model'))
{
	class Documento_model extends Meumodelo
	{
		...
		public function Ajustar($salvar = false)
		{
			if($salvar)
			{
				if(Componente::emptyData($this->data))
					$this->data = date("Y-m-d");
				else
					$this->data = date("Y-m-d", Componente::TimeData($this->data));
			}
			else
			{
				$this->data = date("d/m/Y", Componente::TimeData($this->data));
			}
		}
		...
		public function GetDIR($caminho = "")
		{
			...
			$pastadata = date("Y-m", Componente::TimeData($this->data));
			$aURL = HOST_UPLOADS_PATH."arquivos/{$pasta}/{$pastadata}/{$caminho}";
			return $aURL;
		}
		################################################################################################################
		public function GetDominio($caminho = "")
		{
			$pasta = strtolower($this->Tabela);
			$pastadata = date("Y-m", Componente::TimeData($this->data));
			$aURL = HOST_UPLOADS_URL."arquivos/{$pasta}/{$pastadata}/{$caminho}";
			return $aURL;
		}
		################################################################################################################
        ...
        public function GetDocumento()
        {
            if(empty($this->documento))
                return "";
            if(!$this->FileExiste($this->documento,"", false))
                return "";
            return $this->GetDominio($this->documento);
        }
        ###################################################################################################################
        public function ApagarDocumento()
        {
            if(empty($this->documento))
                return;
            $file = $this->GetDIR($this->documento);
        ...
        public function GetExtensao()
        {
            if(empty($this->documento))
                return;
            $file = $this->GetDIR($this->documento);           
            return Componente::Extensao($file);
        }
